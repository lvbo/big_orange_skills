...
2. 检查 `journals-morning/YYYYMMDD.md`：
   - 不存在时，读取晨间日记模板，保留无关标签及顺序，删除模板 `tags` 列表中语义匹配 `#day/NNNN`、`N月`/`NN月`、`yearAndmonth/NNNNNN` 的旧标签，再各添加一次 `#day/MMDD`、`M月`、`yearAndmonth/YYYYMM` 后创建文件。
   - `#day/MMDD` 在 YAML frontmatter 中必须写成带双引号的字符串，例如日期为 8 月 16 日时写为 `  - "#day/0816"`。不要写成 `  - #day/0816`；未加引号的 `#` 会被 YAML 当作注释，Obsidian 无法把它识别为标签。
   - 月份和年月标签继续使用模板兼容格式，例如 `  - 8月`、`  - yearAndmonth/202608`。标签必须写入 frontmatter 的 `tags` 列表，不要仅在正文中追加同名文本来通过检查。
...
5. 重新读取两个文件并验证：文件均存在；新建晨记的 YAML frontmatter 可正常解析，`tags` 是列表，并且解析后的列表中 `#day/MMDD`、`M月`、`yearAndmonth/YYYYMM` 各恰好出现一次；当日日记的 `## 晨间日记` 章节内目标链接恰好出现一次（其他章节的合法引用不计入）；除计划内位置外没有改动。不能只用全文搜索或 `grep` 看到 `#day/MMDD` 就判定成功，因为未加引号的 YAML 注释也会造成这种假象。
